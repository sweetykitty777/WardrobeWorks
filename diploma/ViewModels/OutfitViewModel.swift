...
class OutfitViewModel: ObservableObject {
    ...
    init() {
        loadMockData() // ✅ Загружаем тестовые данные
    }

    func addOutfit(name: String, outfitItems: [OutfitItem], wardrobe: Wardrobe? = nil, imageName: String? = nil) {
        let newOutfit = Outfit(name: name, outfitItems: outfitItems, wardrobe: wardrobe, imageName: imageName)
        outfits.append(newOutfit)
        print("✅ Аутфит сохранён: \(newOutfit.name), изображение: \(imageName ?? "нет")")
    }
